...
class SaveSchemasDialog(w.Dialog):
    ...
    def __init__(self, master, schemas, **options):
        ...
        self._filename.set(self._schemas_file)
...
class LoadConfigurationsDialog(w.Dialog):
    ...
    def __init__(self, master, **options):
        ...
        self._filename = tk.StringVar()
        self._filename.set(self._configs_file)
        tk.Entry(self, textvariable=self._filename).grid(row=0, column=1, sticky=tk.NW)
        tk.Button(self, text="Select file", command=self.get_filename).grid(row=0, column=3, sticky=tk.NW)       
...
class ConfigurationsManager(tk.Frame):
...
def start_full_manager(**options):
    global root, acme
    
    root = tk.Tk()
    acme = FullManager(root, configs=options.get('configs'))
    acme.pack(fill=tk.BOTH, expand=True)
    root.mainloop()


def start_schemas_manager():
    global root, acme
...
def start_configuration_manager(config, **options):
    global root, acme
    
    root = tk.Tk()
    acme = ConfigurationManager(root, config)
    acme.pack(fill=tk.BOTH, expand=True)
    root.mainloop()


def start_configurations_manager(configs, **options):
    global root, acme
    
    root = tk.Tk()
    acme = ConfigurationsManager(root, configs=configs)
    acme.pack(fill=tk.BOTH, expand=True)
    root.mainloop()
